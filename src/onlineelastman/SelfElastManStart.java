	...
	public SelfElastManStart(int timerWindow) throws MatlabInvocationException {
		...
		try {
			...
			targetThroughput = Integer.parseInt(properties.targetThroughput
					.trim());
			MIN_NUMBER_OF_SERVERS = Integer.parseInt(properties.minServers
					.trim());
			MAX_NUMBER_OF_SERVERS = Integer.parseInt(properties.maxServers
					.trim());
			...
			//if (!CONTROLLER_DISABLED) {
				...
				try {
					...
					if (roperations == 0 && woperations == 0) {
						...
						/*
						 * System.out .println(
						 * "Timer Window just before calculating throughput: " +
						 * timerWindow);
						 */
						...
						OnlineModelMetrics omm = new OnlineModelMetrics(
								fineRead, fineWrite, fineDataSize, rPercentile,
					...
					/*if (writeData.length > 5) {
						...
						// For Debugging
						String ww = "\nWeights:";
						for (Entry<Integer, Integer> entry : wweights
								.entrySet()) {
							ww += "\t" + entry.getValue();
						}
						log.debug(ww);
					...
					/*if (reads.length > 0) {
						...
						if (primalVariables.length != 0) {
							...
							extraServers = Actuator.getExtraNumberOfServers(
									...
									wpredictedValue, NUMBER_OF_SERVERS,
									dataSize);
...
*///End of prediction and actuation commenting
		...
		/*	} else {
				...
				log.info("Data rebalance finished...Enabling the controller");
			} */
		}
